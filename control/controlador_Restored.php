...
if (!empty($_POST["recuperar"])) {
    ...
    if (
        ...
        if (!preg_match("/^[a-zA-ZÀ-ÿ\s]{4,12}$/", $_POST["respuesta1"])) {
        ...
        } elseif (!preg_match("/^[a-zA-ZÀ-ÿ\s]{4,12}$/", $_POST["respuesta2"])) {
            echo '<div class="alerta">La respuesta 2 debe contener solo letras y espacios (mínimo 4 caracteres)</div>';
            ...
            if ($_POST["nueva_clave"] !== $_POST["confirmar_contrasenia"]) {
                ...
                $new_contrasenia = trim($_POST["new_contrasenia"]);
                ...
                try {
                    ...
                    if ($stmt->fetch(PDO::FETCH_OBJ)) {
                        ...
                        $update_sql = "UPDATE usuarios SET contrasenia = :contrasenia WHERE respuesta1 = :respuesta1 AND respuesta2 = :respuesta2";
