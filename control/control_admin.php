...
if (!empty($_POST["admin_login"])) {
    ...
    if (empty($_POST["correo"]) || empty($_POST["contrasenia"])) {
    ...
    } elseif (!preg_match("/^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d]{8,20}$/", $_POST["contrasenia"])) {
        // Validación de clave: entre 8 y 20 caracteres, al menos una letra y un número
        ...
        try {
            ...
            $sql = "SELECT * FROM admint WHERE correo = :correo";
            ...
            if ($datos = $stmt->fetch(PDO::FETCH_OBJ)) {
                ...
                echo '<div class="alerta">Administrador no encontrado</div>';
            }
